...
// @ts-ignore: Suppress spellcheck and missing module errors for localized strings and stubs
import { isApiConfigured, streamChatCompletion, getChatTitle, generateImageFromPrompt, analyzeImageWithAzure, analyzeDocumentWithAzure } from '../src/services/openaiService';
import type { Message, ChatSession, Attachment } from '../types';
import { useAppContext } from '../context/AppContext';
import { PlusIcon, ChatBubbleIcon, SendIcon, MenuIcon, UserCircleIcon, SparklesIcon, PlanIcon, CustomizeIcon, HelpIcon, LogoutIcon, ImageIcon, GlobeIcon, PencilIcon, ResearchIcon, LightbulbIcon, MicrophoneIcon, ClipboardIcon, ThumbsUpIcon, ThumbsDownIcon, ArrowPathIcon, ShareIcon, ClipboardCheckIcon, PaperClipIcon, SpeakerWaveIcon, StopCircleIcon, XMarkIcon, ArchiveBoxIcon, TrashIcon, ArrowUpOnSquareIcon, XCircleIcon, ShieldCheckIcon } from '../components/icons';
import SettingsModal from '../components/SettingsModal';
import type { ActiveTab as SettingsTab } from '../components/SettingsModal';
import { findLastIndex } from '../src/utils/helpers';

// Type stubs for missing modules (remove if you have real modules)
declare module '../src/services/openaiService' {
// @ts-ignore: Suppress spellcheck for localized strings
const translations: {
  ...
  en: {
    ...
    upgradePlanDesc: "More access to the best models",
  ...
  ar: {
    ...
    writeCode: "كتابة أو برمجة",
    runResearch: "إجراء بحث معمق",
    thinkLonger: "فكر لوقت أطول",
    copy: "نسخ",
    copied: "تم النسخ!",
    goodResponse: "رد جيد",
    badResponse: "رد سيء",
    ...
    logout: "تسجيل الخروج",
    chats: "الدردشات",
    const t: Record<string, string> = translations[language as keyof typeof translations];
    myPlan: "خطتي",
    customize: "الإعدادات",
    helpCenter: "مركز المساعدة",
    adminDashboard: "لوحة تحكم الأدمن",
...
const ChatInput: React.FC<{
    onSendMessage: (message: string, file: File | null, toolOverride?: string | null) => void;
    isLoading: boolean;
    activeTool: string | null;
    setActiveTool: (tool: string | null) => void;
}> = ({ onSendMessage, isLoading, activeTool, setActiveTool }) => {
    ...
    const placeholderText = useMemo(() => {
        switch(activeTool) {
            case 'image': return t.createImage + '...';
            case 'searchWeb': return t.searchWeb + '...';
            case 'writeCode': return t.writeCode + '...';
        recognizer.recognizing = (_s: any, e: any) => {
            setInput(finalTranscript + e.result.text);
        };
    ...
    const ToolMenuItem: React.FC<{icon: React.ReactNode, text: string, tool: string}> = ({icon, text, tool}) => (
        <button onClick={() => { setActiveTool(tool); setIsToolsOpen(false); }} className="w-full text-start flex items-center gap-3 px-3 py-2 rounded-md text-sm hover:bg-isocio-gray-100 dark:hover:bg-isocio-gray-800 transition-colors">
            {icon}
            <span>{text}</span>
        </button>
    );
        ...
        <div className="w-full max-w-3xl mx-auto">
            <form onSubmit={handleSend} className="relative">
                ...
                <div className="flex items-end gap-2 p-2 border border-isocio-gray-200 dark:border-isocio-gray-700 rounded-2xl focus-within:ring-2 focus-within:ring-isocio-blue transition-shadow bg-isocio-gray-100 dark:bg-isocio-gray-800">
                    ...
                    <div ref={toolsMenuRef} className="relative flex-shrink-0">
                         <div className={`absolute bottom-full mb-2 w-64 bg-white dark:bg-isocio-gray-900 rounded-xl shadow-lg border border-isocio-gray-200 dark:border-isocio-gray-700 z-50 p-2 space-y-1 transition-all duration-200 ease-out origin-bottom-left ${isToolsOpen ? 'opacity-100 translate-y-0 scale-100' : 'opacity-0 translate-y-2 scale-95 pointer-events-none'}`}>
                            <ToolMenuItem icon={<ImageIcon className="w-5 h-5 text-isocio-gray-500" />} text={t.createImage} tool="image" />
                            <ToolMenuItem icon={<GlobeIcon className="w-5 h-5 text-isocio-gray-500" />} text={t.searchWeb} tool="searchWeb" />
                            <ToolMenuItem icon={<PencilIcon className="w-5 h-5 text-isocio-gray-500" />} text={t.writeCode} tool="writeCode" />
                            <ToolMenuItem icon={<ResearchIcon className="w-5 h-5 text-isocio-gray-500" />} text={t.runResearch} tool="runResearch" />
                            <ToolMenuItem icon={<LightbulbIcon className="w-5 h-5 text-isocio-gray-500" />} text={t.thinkLonger} tool="thinkLonger" />
                        </div>
                        <button type="button" onClick={() => setIsToolsOpen(p => !p)} title={t.tools} className="flex items-center gap-2 p-2 hover:bg-isocio-gray-200 dark:hover:bg-isocio-gray-700 rounded-lg">
                            <PlusIcon className="w-5 h-5 text-isocio-gray-600 dark:text-isocio-gray-400" />
                            <span className="text-sm font-medium text-isocio-gray-600 dark:text-isocio-gray-400 hidden sm:inline">{t.tools}</span>
                        </button>
                    </div>
                 ...
                 {activeTool && (
                    <div className="flex justify-center items-center gap-2 text-xs text-isocio-gray-500 dark:text-isocio-gray-400 mt-2">
                        <span className="bg-isocio-gray-200 dark:bg-isocio-gray-700 px-2 py-1 rounded-full">
                            Tool: <strong>{activeTool}</strong>
                        </span>
                        <button onClick={() => setActiveTool(null)} className="p-1 rounded-full hover:bg-isocio-gray-200 dark:hover:bg-isocio-gray-700">
                           <XMarkIcon className="w-4 h-4" />
                        </button>
                    </div>
                )}
            </form>
            {!activeTool && <p className="text-center text-xs text-isocio-gray-500 dark:text-isocio-gray-400 mt-2 px-4">{t.disclaimer}</p>}
        </div>
    );
};
...
const MessageActions: React.FC<{ 
    ...
    const handleCopy = () => {
        ...
        setCopied(true);
}> = ({ messages, isLoading, onSendMessage, onRegenerate, sessionActive: _sessionActive, onPlayText, speakingMessageId }) => {
  const { language } = useAppContext();
  const t: Record<string, string> = translations[language as keyof typeof translations];
...
const ChatView: React.FC<{
  ...
  onPlayText: (message: Message) => void;
  speakingMessageId: string | null;
}> = ({ messages, isLoading, onSendMessage, onRegenerate, sessionActive, onPlayText, speakingMessageId }) => {
  const { language } = useAppContext();
  const t = translations[language];
...
const ChatPage: React.FC<{ onLogout: () => void; }> = ({ onLogout }) => {
  ...
  const loadSessionsFromStorage = useCallback(() => {
    try {
      ...
      if (savedSessions) {
        ...
        // Find the most recent non-archived chat to be active
        const lastActive = parsedSessions.filter(s => !s.isArchived).sort((a,b) => b.createdAt - a.createdAt)[0];
        if (lastActive) {
  ...
  const createNewChat = () => {
      getChatTitle(input).then((title: string) => {
        updateChatSessions(prev => prev.map(s => s.id === activeChatId ? {...s, title} : s));
      });
      id: `chat_${Date.now()}`,
      title: t.newChat,
      messages: [],
  ...
  const selectChat = (id: string) => {
    ...
    if(session?.isArchived) setCurrentView('archived');
              ...
              prev.map(s => s.id === currentChatId ? { ...s, messages: s.messages.map((m: Message) => m.id === modelMessage.id ? {...m, content: imageContent} : m) } : s)

  const handleSendMessage = async (input: string, file: File | null) => {
    if (!isServiceConfigured) return;
  
    const attachment: Attachment | null = await processFile(file);
  
    // Pre-create the user message with potential analysis text
    let userMessageContent = input;
    if (attachment && (attachment.type === 'image' || attachment.type === 'document')) {
        // The analysis result is now in attachment.content for these types
        userMessageContent = `${attachment.content}\n\n${input}`;
    }
    ...
    let currentChatId = activeChatId;
                    ? { ...s, messages: s.messages.map((m: Message) => m.id === modelMessage.id ? { ...m, content: m.content + chunk } : m) }
    if (!currentChatId) {
      const newSession: ChatSession = {
        id: `chat_${Date.now()}`,
        title: input.substring(0, 30) || t.newChat,
        messages: [userMessage],
        createdAt: Date.now(),
      };
      updateChatSessions(prev => [newSession, ...prev]);
      setActiveChatId(newSession.id);
          ? { ...s, messages: s.messages.map((m: Message) => m.id === modelMessage.id ? { ...m, content: `Error: ${error instanceof Error ? error.message : "An unknown error occurred."}` } : m) }
    } else {
      updateChatSessions(prev =>
        prev.map(s => s.id === currentChatId ? { ...s, messages: [...s.messages, userMessage] } : s)
      );
    }
    
    setIsLoading(true);

    if (activeChatId && chatSessions.find(s => s.id === activeChatId)?.messages.length === 1) {
      getChatTitle(input).then(title => {
        updateChatSessions(prev => prev.map(s => s.id === activeChatId ? {...s, title} : s));
      });
    }

    const lastUserMessageIndex = findLastIndex(currentSession.messages, (msg: Message) => msg.role === 'user');
      id: `model_${Date.now()}`,
      role: 'model',
      content: '',
      timestamp: new Date().toISOString(),
    ...
    updateChatSessions(prev =>
        prev.map(s => s.id === currentChatId ? { ...s, messages: [...s.messages, modelMessage] } : s)
    );

    try {
      if (activeTool === 'image') {
          const b64Json = await generateImageFromPrompt(input);
          const imageContent = `![Generated image: ${input}](data:image/png;base64,${b64Json})`;
          updateChatSessions(prev =>
              prev.map(s => s.id === currentChatId ? { ...s, messages: s.messages.map(m => m.id === modelMessage.id ? {...m, content: imageContent} : m) } : s)
          );
      } else {
        // The history needs to be fetched from the state *after* the user message has been added
        const updatedSession = chatSessions.find(s => s.id === currentChatId);
        const history = updatedSession ? updatedSession.messages.slice(0, -1) : []; // Send all messages except the empty model message
        
        abortControllerRef.current = new AbortController();
        const stream = await streamChatCompletion(history, activeTool);
        ...
        while (!done) {
            ...
            updateChatSessions(prev =>
              ...
              const { caption: _caption, text: _text } = await analyzeImageWithAzure(arrayBuffer);
      updateChatSessions(prev =>
        prev.map(s => s.id === currentChatId
          ? { ...s, messages: s.messages.map(m => m.id === modelMessage.id ? { ...m, content: `Error: ${error instanceof Error ? error.message : "An unknown error occurred."}` } : m) }
          : s
        )
      );
    } finally {
  ...
  const handleRegenerate = () => {
    ...
    updateChatSessions(prev => prev.map(s => {
      if (s.id === activeChatId) {
        return { ...s, messages: historyToResend };
      }
      return s;
    }));
  
    // We need to wait for state to update before sending the message
    // A small timeout is a simple way to achieve this.
    setTimeout(() => {
        const userMessage = historyToResend[historyToResend.length-1];
        // This is a bit tricky, the original attachment file is lost.
        // We will proceed with text content only for regeneration.
        handleSendMessage(userMessage.content, null); 
    }, 100);
  };
  ...
  const processFile = async (file: File | null): Promise<Attachment | null> => {
      ...
      try {
          if (file.type.startsWith('image/')) {
              ...
              const { caption, text } = await analyzeImageWithAzure(arrayBuffer);
              ...
              return {
                  ...
                  type: 'image',
                  mimeType: file.type,
                  content: dataUrl, // The data URL for display
              };
          ...
          updateChatSessions(() => []);
                  ...
                  type: 'document',
                  mimeType: file.type,
                  content: `${t.documentAnalysisPrefix}\n\n${markdownContent}`,
              };
  ...
  return (
    <div className="flex h-screen overflow-hidden">
      <Sidebar
        ...
        onNavigateToView={(view: ViewType) => {
            setCurrentView(view);
        ...
        <div className="flex-shrink-0 p-4 pt-2">
           {isServiceConfigured ? (
             <>
               ...
               <ChatInput onSendMessage={handleSendMessage} isLoading={isLoading} activeTool={activeTool} setActiveTool={setActiveTool} />
             </>
           ) : (
